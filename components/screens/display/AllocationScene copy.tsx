...
// Fully imperative: exposed via ref so parent never re-renders this component.
interface EngineCoreHandle {
  pulse: () => void;
}
const EngineCore = React.memo(React.forwardRef<EngineCoreHandle, { color: string; rgb: string }>(
  function EngineCore({ color, rgb }, ref) {
    ...
    React.useImperativeHandle(ref, () => ({
      pulse() {
        // Speed up arcs briefly
        if (arc1Ref.current) arc1Ref.current.style.animationDuration = "1.1s";
        if (arc2Ref.current) arc2Ref.current.style.animationDuration = ".75s";
        // Intensify dot glow
        if (dotRef.current)  dotRef.current.style.boxShadow = `0 0 22px ${color}`;
        // Fire burst
        const burst = burstRef.current;
        if (burst) {
          burst.style.animation = "none";
          void burst.offsetWidth;
          burst.style.animation = "engCoreBurst .7s cubic-bezier(.2,.8,.2,1) forwards";
        }
        // Reset after burst duration
        setTimeout(() => {
          if (arc1Ref.current) arc1Ref.current.style.animationDuration = "5s";
          if (arc2Ref.current) arc2Ref.current.style.animationDuration = "3.4s";
          if (dotRef.current)  dotRef.current.style.boxShadow = `0 0 14px ${color}`;
        }, 700);
      },
    }));
    ...
    return (
      <div aria-hidden="true" style={{
        ...
        <div ref={arc1Ref} style={{
          ...
          animation:"engCore1 5s linear infinite",
        }}/>
        <div ref={arc2Ref} style={{
          ...
          animation:"engCore2 3.4s linear infinite reverse",
        }}/>
        <div style={{
...
export function AllocationFlow({ scene }: { scene: AllocationFlowScene }) {
  ...
  const fcfsCoreRef     = useRef<EngineCoreHandle>(null);
  const fbCoreRef       = useRef<EngineCoreHandle>(null);
  ...
  const scheduleNext = React.useCallback(()=>{
    ...
    tickerRef.current=setTimeout(()=>{
      ...
      (isFcfs ? fcfsCoreRef : fbCoreRef).current?.pulse();
      flyChip(ini,eng);
      setTimeout(()=>{
              ...
              <div style={{display:"grid",gridTemplateRows:"1fr 1fr",gap:10,minHeight:0}}>
                {engines.map((eng,ei)=>(
                  <div key={eng.key} ref={eng.key==="fcfs"?fcfsEngineRef:fbEngineRef}
                    ...
                    <EngineCore ref={eng.key==="fcfs"?fcfsCoreRef:fbCoreRef} color={eng.color} rgb={eng.rgb}/>
                    <div>
