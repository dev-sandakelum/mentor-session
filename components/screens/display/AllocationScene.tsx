...
// ─── AllocationLoadScene — self-assembling zero state ────────────────────────
...
export function AllocationLoadScene() {
  ...
  // Minimum-delay gate: shows the robot-face opening for at least 4s before
  // the self-assembly animation is allowed to start.
  useEffect(() => {
    const t = setTimeout(() => setMinDelayDone(true), 4000);
    return () => clearTimeout(t);
  }, []);

  // Self-assembly only starts once the robot-face gate has lifted — otherwise
  // `assembled` would flip true (and its CSS transitions would begin) while
  // the robot face is still covering the screen, wasting the transition and
  // making the dashboard "pop" fully in the instant the gate lifts instead of
  // animating in.
  useEffect(() => {
    if (!minDelayDone) return;
    let secondFrame = 0;
    const firstFrame = requestAnimationFrame(() => {
      secondFrame = requestAnimationFrame(() => setAssembled(true));
    });
    return () => {
  ...
  useEffect(() => {
    fetch("/api/display/mentors")
      ...
      .then((data: { mentors?: { name: string; capacity?: number }[] }) => {
        setMentors((data.mentors ?? []).map(mentor => ({
          name: mentor.name,
          allocated: 0,
    ...
    fetch("/api/display/registrations")
      ...
      .then((data: unknown) => {
        ...
        const possibleTotal = [record.menteeTotal, record.total, record.count, Array.isArray(record.registrations) ? record.registrations.length : undefined]
          .find(value => typeof value === "number" && Number.isFinite(value));
        if (typeof possibleTotal === "number") setTotal(possibleTotal);
      })
      .catch(() => undefined);
  }, []);
  ...
  const fcfsPct = 0;
  const fbPct = 0;
  const activeEngine: "fcfs" | "fallback" | null = null;
  const isComplete = false;
  const displayed: { mentee: string; mentor: string; method: "preference" | "fallback" | "manual"; priority: number | null }[] = [];
  const totalLabel: number | string = total > 0 ? total : "—";
  ...
  const methodColor = (method: string, priority: number | null) => {
    if (method === "preference") return priority === 1 ? "#22c55e" : priority === 2 ? "#f59e0b" : "#a78bfa";
    return "#38bdf8";
  };
  ...
  // ── Opening sequence — large robot face for the first 4s ──────────────────
  // This is the very first thing the user sees after pressing "Engine Ready".
  // It gates the self-assembly animation below (see the `minDelayDone` effect
  // above), so there is no overlap between this screen and the dashboard
  // assembling itself — one replaces the other cleanly.
  if (!minDelayDone) {
    return (
      <div style={{ position:"fixed", inset:0, overflow:"hidden", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:'"DM Sans","Manrope",system-ui,sans-serif', WebkitFontSmoothing:"antialiased", color:"#eef5ff", background:"radial-gradient(circle at 10% 10%,rgba(45,108,240,.28),transparent 34rem),radial-gradient(circle at 90% 15%,rgba(18,182,221,.16),transparent 30rem),radial-gradient(circle at 50% 110%,rgba(79,157,255,.12),transparent 40rem),linear-gradient(150deg,#03081a 0%,#061131 55%,#04091c 100%)" }}>
        ...
        {/* Drifting orbs */}
        <div style={{ position:"fixed", width:520, height:520, borderRadius:"50%", filter:"blur(100px)", opacity:.18, left:-160, top:"20%", background:"#4f9dff", animation:"rf-drift1 9s ease-in-out infinite alternate", pointerEvents:"none" }} />
        <div style={{ position:"fixed", width:480, height:480, borderRadius:"50%", filter:"blur(100px)", opacity:.16, right:-160, bottom:"10%", background:"#5ee1ff", animation:"rf-drift2 11s ease-in-out infinite alternate", pointerEvents:"none" }} />
        ...
        <div style={{ position:"relative", display:"flex", flexDirection:"column", alignItems:"center", gap:"clamp(18px,2.4vh,32px)", textAlign:"center", padding:"0 24px" }}>
          ...
          <div style={{ fontSize:"clamp(13px,1.15vh,17px)", fontWeight:600, color:"#8aa3cc", letterSpacing:".04em" }}>
        ...
        <style>{`
          ...
          @keyframes rf-drift1 { to{transform:translate3d(40px,-30px,0) scale(1.1)} }
          @keyframes rf-drift2 { to{transform:translate3d(-36px,26px,0) scale(1.08)} }
        `}</style>
      </div>
    );
  }
  ...
  return (
    <div style={{ position:"fixed", inset:0, overflow:"hidden", fontFamily:'"DM Sans","Manrope",system-ui,sans-serif', WebkitFontSmoothing:"antialiased", color:"#eef5ff", background:"radial-gradient(circle at 10% 10%,rgba(45,108,240,.28),transparent 34rem),radial-gradient(circle at 90% 15%,rgba(18,182,221,.16),transparent 30rem),radial-gradient(circle at 50% 110%,rgba(79,157,255,.12),transparent 40rem),linear-gradient(150deg,#03081a 0%,#061131 55%,#04091c 100%)" }}>
      ...
      <div style={{ position:"relative", height:"100vh", width:"min(1720px,100%)", margin:"0 auto", padding:"clamp(10px,1.3vh,20px) clamp(20px,3vw,52px)", display:"flex", flexDirection:"column", gap:"clamp(8px,1vh,16px)" }}>
        ...
        {/* Header */}
        <header style={{ ...assemblyStyle("translateY(-16px)", 0, .45), display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, border:"1px solid rgba(140,190,255,.13)", background:"rgba(8,20,50,.7)", padding:"clamp(8px,.8vh,13px) clamp(14px,1.3vh,20px)", borderRadius:999, color:"#fbbf24", fontSize:"clamp(13px,1.15vh,17px)", fontWeight:700, letterSpacing:".1em", textTransform:"uppercase" }}>
            <span style={{ width:10, height:10, borderRadius:"50%", background:"#fbbf24", boxShadow:"0 0 10px rgba(251,191,36,.65)", animation:"alloc-ready-pulse 2.2s ease-in-out infinite", flexShrink:0, display:"inline-block" }} />
            Engine Ready
          </div>
        </header>
        ...
        <div style={{ flex:1, minHeight:0, display:"grid", gridTemplateColumns:"minmax(0,1fr) clamp(320px,24vw,420px)", gap:"clamp(8px,1vh,16px)" }}>
          ...
          <div style={{ ...assemblyStyle("scale(.985)", .08, .5), position:"relative", minHeight:0, border:"1px solid rgba(140,190,255,.13)", background:"linear-gradient(145deg,rgba(13,30,70,.82),rgba(6,14,38,.86))", boxShadow:"0 30px 90px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.04)", backdropFilter:"blur(20px)", borderRadius:28, padding:"clamp(14px,1.5vh,24px)", display:"flex", flexDirection:"column" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:14, paddingBottom:"clamp(10px,1.2vh,18px)", flexShrink:0 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:9, color:"#7dd3fc", fontSize:"clamp(13px,1.1vh,16px)", fontWeight:700, padding:"clamp(6px,.65vh,10px) clamp(12px,1.2vh,18px)", border:"1px solid rgba(125,211,252,.25)", borderRadius:999, background:"rgba(56,189,248,.1)" }}>
                <div style={{ width:8, height:8, borderRadius:"50%", background:"#7dd3fc", boxShadow:"0 0 8px rgba(125,211,252,.65)", flexShrink:0 }} />
                Standby
              </div>
            </div>
            ...
            <div style={{ position:"relative", flex:1, minHeight:0, display:"grid", gridTemplateColumns:"minmax(180px,.78fr) 52px minmax(260px,1.4fr) 52px minmax(200px,.9fr)", alignItems:"stretch", gap:10 }}>
              ...
              {/* Queue node */}
              <div ref={queueNodeRef} style={{ ...assemblyStyle("translateX(-20px)", .18), position:"relative", border:"1px solid rgba(140,190,255,.1)", background:"rgba(3,10,30,.5)", borderRadius:22, padding:"clamp(14px,1.5vh,22px)", overflow:"hidden", display:"flex", flexDirection:"column", justifyContent:"space-between" }}>
                <div>
                ...
                <div style={{ display:"flex", alignItems:"flex-end", gap:10 }}>
                  <strong style={{ font:`800 clamp(34px,3.8vh,58px)/.9 Manrope,sans-serif`, letterSpacing:"-.05em", color:"#fff" }}>{total > 0 ? total : "—"}</strong>
                  <span style={{ fontSize:"clamp(14px,1.2vh,18px)", color:"#7d97c2", paddingBottom:6 }}>waiting</span>
                </div>
              </div>

              {/* Connector */}
              <div style={{ ...assemblyStyle("scaleX(0)", .28, .48), transformOrigin:"left center", position:"relative", display:"flex", alignItems:"center" }}>
                ...
                <div style={{ position:"absolute", width:10, height:10, borderRadius:"50%", background:"#4f9dff", boxShadow:"0 0 14px #4f9dff", left:"50%", animation:"alloc-connector-pulse 2s ease-in-out infinite" }} />
              </div>
              ...
              <div style={{ display:"grid", gridTemplateRows:"1fr 1fr", gap:10, minHeight:0 }}>
                {engines.map(eng => (
                  <div key={eng.key} ref={eng.key==="fcfs" ? fcfsEngineRef : fbEngineRef}
                    style={{ ...assemblyStyle("translateY(18px)", eng.key === "fcfs" ? .36 : .46), position:"relative", border:`1px solid ${eng.border}`, background:`linear-gradient(115deg,${eng.bg},rgba(6,16,45,.4))`, borderRadius:20, padding:"clamp(12px,1.3vh,20px) clamp(14px,1.4vh,22px)", overflow:"hidden", display:"flex", flexDirection:"column", justifyContent:"space-between" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:12, marginTop:"clamp(8px,.9vh,14px)" }}>
                      ...
                      <strong style={{ font:`800 clamp(15px,1.4vh,20px) Manrope,sans-serif`, minWidth:"clamp(46px,4.2vh,64px)", textAlign:"right", color:"#eef5ff" }}>
                        {eng.count}<span style={{ color:"#6f89b3", fontWeight:600 }}>/{totalLabel}</span>
                      </strong>
                    </div>
                  </div>
                ))}
              ...
              {/* Connector */}
              <div style={{ ...assemblyStyle("scaleX(0)", .54, .48), transformOrigin:"left center", position:"relative", display:"flex", alignItems:"center" }}>
                <div style={{ height:2, width:"100%", background:"linear-gradient(90deg,rgba(79,157,255,.12),rgba(79,157,255,.7),rgba(79,157,255,.12))" }} />
                <div style={{ position:"absolute", right:0, width:9, height:9, borderTop:"2px solid #4f9dff", borderRight:"2px solid #4f9dff", transform:"rotate(45deg)" }} />
                <div style={{ position:"absolute", width:10, height:10, borderRadius:"50%", background:"#4f9dff", boxShadow:"0 0 14px #4f9dff", left:"50%", animation:"alloc-connector-pulse 2s ease-in-out infinite", animationDelay:"-.7s" }} />
              </div>

              {/* Latest matches */}
              <div ref={assignedNodeRef} style={{ ...assemblyStyle("translateX(20px)", .62), border:"1px solid rgba(140,190,255,.1)", background:"rgba(3,10,30,.5)", borderRadius:22, padding:"clamp(14px,1.5vh,22px)", overflow:"hidden", display:"flex", flexDirection:"column", minHeight:0 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:6, overflow:"hidden", flex:1, minHeight:0 }}>
                  {displayed.length === 0 && (
                    <div style={{ flex:1, display:"grid", placeItems:"center", color:"#607ba5", fontSize:"clamp(12px,1.05vh,15px)", textAlign:"center", padding:12 }}>Waiting for first match…</div>
                  )}
                  {displayed.map((row, i) => (
            ...
            <div style={{ flexShrink:0, display:"grid", gridTemplateColumns:"1fr auto", gap:18, alignItems:"center", borderTop:"1px solid rgba(140,190,255,.13)", padding:"clamp(10px,1.1vh,16px) 3px 2px", marginTop:"clamp(10px,1.1vh,16px)" }}>
              ...
              <div style={{ font:`700 clamp(15px,1.3vh,20px) Manrope,sans-serif`, color:"#8aa3cc", whiteSpace:"nowrap" }}>
                <strong style={{ color:"#eef5ff", fontSize:"clamp(20px,1.8vh,28px)" }}>{revealedCount}</strong>/{totalLabel}
          ...
          <div style={{ display:"grid", gridTemplateRows:"auto minmax(0,1fr)", gap:"clamp(8px,1vh,16px)", minHeight:0 }}>
                ...
                <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
                  <div>
                    ...
                    <div style={{ font:`800 clamp(42px,4.2vh,66px)/1 Manrope,sans-serif`, letterSpacing:"-.05em" }}>
                      {revealedCount}<small style={{ fontSize:"clamp(18px,1.6vh,24px)", color:"#6f89b3", fontWeight:600, letterSpacing:0 }}>/{totalLabel}</small>
                    </div>
                  </div>
            ...
            <div style={{ ...assemblyStyle("translateY(16px)", .70, .6), border:"1px solid rgba(140,190,255,.13)", background:"linear-gradient(145deg,rgba(13,30,70,.82),rgba(6,14,38,.86))", boxShadow:"0 30px 90px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.04)", backdropFilter:"blur(20px)", borderRadius:24, padding:"clamp(14px,1.4vh,22px)", display:"flex", flexDirection:"column", minHeight:0, overflow:"hidden" }}>
              ...
              <div style={{ flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between", paddingBottom:"clamp(10px,1vh,15px)", borderBottom:"1px solid rgba(140,190,255,.13)", marginBottom:"clamp(8px,.8vh,12px)" }}>
                <div style={{ font:`800 clamp(14px,1.3vh,18px) Manrope,sans-serif` }}>
                  Mentor load · <span style={{ color:"#6f89b3", fontWeight:600 }}>{mentors.length} mentors</span>
                </div>
              ...
              <div style={{ flex:1, minHeight:0, overflow:"hidden" }}>
                {mentors.length === 0 ? (
                  ...
                  <div className={assembled ? "ald-grid-wave ald-grid-wave--in" : "ald-grid-wave"} style={{ height:"100%" }}><MentorLoadGrid mentors={mentors} hitName={null} /></div>
                )}
              </div>

              {/* Footer stats */}
              {mentors.length > 0 && (() => {
                const assigned = mentors.filter(m => m.allocated > 0);
                const idle     = mentors.length - assigned.length;
                const total_a  = mentors.reduce((s, m) => s + m.allocated, 0);
                const maxLoad  = assigned.length > 0 ? Math.max(...assigned.map(m => m.allocated)) : 0;
                const avg      = (total_a / mentors.length).toFixed(1);
      ...
      <style>{`
        ...
        @keyframes alloc-ready-pulse { 50%{opacity:.55;box-shadow:0 0 16px rgba(251,191,36,.85)} }
        @keyframes alloc-connector-pulse { 0%,100%{opacity:.55;transform:scale(.82)}50%{opacity:1;transform:scale(1.18)} }
        @keyframes ald-tile-in { from{opacity:0;transform:scale(.6)}to{opacity:1;transform:scale(1)} }
        .ald-grid-wave > div > div { opacity:0; }
        .ald-grid-wave--in > div > div { animation:ald-tile-in .45s cubic-bezier(.16,1,.3,1) both; }
        .ald-grid-wave--in > div > div:nth-child(1) { animation-delay:0ms; }
        .ald-grid-wave--in > div > div:nth-child(2) { animation-delay:30ms; }
        .ald-grid-wave--in > div > div:nth-child(3) { animation-delay:60ms; }
        .ald-grid-wave--in > div > div:nth-child(4) { animation-delay:90ms; }
        .ald-grid-wave--in > div > div:nth-child(5) { animation-delay:120ms; }
        .ald-grid-wave--in > div > div:nth-child(6) { animation-delay:150ms; }
        .ald-grid-wave--in > div > div:nth-child(7) { animation-delay:180ms; }
        .ald-grid-wave--in > div > div:nth-child(8) { animation-delay:210ms; }
        .ald-grid-wave--in > div > div:nth-child(9) { animation-delay:240ms; }
        .ald-grid-wave--in > div > div:nth-child(10) { animation-delay:270ms; }
        .ald-grid-wave--in > div > div:nth-child(11) { animation-delay:300ms; }
        .ald-grid-wave--in > div > div:nth-child(12) { animation-delay:330ms; }
        .ald-grid-wave--in > div > div:nth-child(13) { animation-delay:360ms; }
        .ald-grid-wave--in > div > div:nth-child(14) { animation-delay:390ms; }
        .ald-grid-wave--in > div > div:nth-child(15) { animation-delay:420ms; }
        .ald-grid-wave--in > div > div:nth-child(16) { animation-delay:450ms; }
        .ald-grid-wave--in > div > div:nth-child(17) { animation-delay:480ms; }
        .ald-grid-wave--in > div > div:nth-child(18) { animation-delay:510ms; }
        .ald-grid-wave--in > div > div:nth-child(19) { animation-delay:540ms; }
        .ald-grid-wave--in > div > div:nth-child(20) { animation-delay:570ms; }
        .ald-grid-wave--in > div > div:nth-child(21) { animation-delay:600ms; }
        .ald-grid-wave--in > div > div:nth-child(22) { animation-delay:630ms; }
        .ald-grid-wave--in > div > div:nth-child(23) { animation-delay:660ms; }
        .ald-grid-wave--in > div > div:nth-child(24) { animation-delay:690ms; }
        .ald-grid-wave--in > div > div:nth-child(25) { animation-delay:720ms; }
        .ald-grid-wave--in > div > div:nth-child(26) { animation-delay:750ms; }
        .ald-grid-wave--in > div > div:nth-child(27) { animation-delay:780ms; }
        .ald-grid-wave--in > div > div:nth-child(28) { animation-delay:810ms; }
        .ald-grid-wave--in > div > div:nth-child(29) { animation-delay:840ms; }
        .ald-grid-wave--in > div > div:nth-child(30) { animation-delay:870ms; }
        .ald-grid-wave--in > div > div:nth-child(31) { animation-delay:900ms; }
        .ald-grid-wave--in > div > div:nth-child(32) { animation-delay:930ms; }
        .ald-grid-wave--in > div > div:nth-child(33) { animation-delay:960ms; }
        .ald-grid-wave--in > div > div:nth-child(34) { animation-delay:990ms; }
        .ald-grid-wave--in > div > div:nth-child(35) { animation-delay:1020ms; }
        .ald-grid-wave--in > div > div:nth-child(36) { animation-delay:1050ms; }
        .ald-grid-wave--in > div > div:nth-child(37) { animation-delay:1080ms; }
        .ald-grid-wave--in > div > div:nth-child(38) { animation-delay:1110ms; }
        .ald-grid-wave--in > div > div:nth-child(39) { animation-delay:1140ms; }
        .ald-grid-wave--in > div > div:nth-child(40) { animation-delay:1170ms; }
        .ald-grid-wave--in > div > div:nth-child(41) { animation-delay:1200ms; }
        .ald-grid-wave--in > div > div:nth-child(42) { animation-delay:1230ms; }
        .ald-grid-wave--in > div > div:nth-child(43) { animation-delay:1260ms; }
        .ald-grid-wave--in > div > div:nth-child(44) { animation-delay:1290ms; }
        .ald-grid-wave--in > div > div:nth-child(45) { animation-delay:1320ms; }
        .ald-grid-wave--in > div > div:nth-child(46) { animation-delay:1350ms; }
        .ald-grid-wave--in > div > div:nth-child(47) { animation-delay:1380ms; }
        .ald-grid-wave--in > div > div:nth-child(48) { animation-delay:1410ms; }
        .ald-grid-wave--in > div > div:nth-child(49) { animation-delay:1440ms; }
        .ald-grid-wave--in > div > div:nth-child(50) { animation-delay:1470ms; }
        .ald-grid-wave--in > div > div:nth-child(51) { animation-delay:1500ms; }
        .ald-grid-wave--in > div > div:nth-child(52) { animation-delay:1530ms; }
        .ald-grid-wave--in > div > div:nth-child(53) { animation-delay:1560ms; }
        .ald-grid-wave--in > div > div:nth-child(54) { animation-delay:1590ms; }
        .ald-grid-wave--in > div > div:nth-child(55) { animation-delay:1620ms; }
        .ald-grid-wave--in > div > div:nth-child(56) { animation-delay:1650ms; }
        .ald-grid-wave--in > div > div:nth-child(57) { animation-delay:1680ms; }
        .ald-grid-wave--in > div > div:nth-child(58) { animation-delay:1710ms; }
        .ald-grid-wave--in > div > div:nth-child(59) { animation-delay:1740ms; }
        .ald-grid-wave--in > div > div:nth-child(60) { animation-delay:1770ms; }
      `}</style>
    </div>
  );
}
