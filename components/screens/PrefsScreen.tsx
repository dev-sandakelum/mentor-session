...
import { MobilePrefsScreen } from "./MobilePrefsScreen";
...
export function PrefsScreen() {
  ...
  const toggleMentor = (id: string, isFull: boolean) => {
    ...
    if (picks.length >= 3)
      return showToast(
        "You already picked 3 mentors — remove one to change your selection."
      );
    setPicks([...picks, id]);
  };
  ...
  return (
    <div className="container">
      ...
      <p className="section-sub">
        Your preferences are processed in submission order. Each mentor can accept only
        ...
        <div className="mentor-grid">
          {loading &&
            Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="mentor-card mentor-card-skeleton"
                aria-hidden="true"
              >
          ...
          {!loading && mentors.length === 0 && (
            <p className="muted">No approved mentors are available yet.</p>
          )}
                ...
                >
                  {picked && (
                    <button
                      ...
                      onClick={() => toggleMentor(picked, false)}
                    >
                      ✕ remove
                    </button>
                  )}
                  ...
                  <div className="slot-name">
                    {pickedMentor?.fullName ?? "Not selected yet"}
                  </div>
                </div>
              );
            })}
            <p className="hint" style={{ margin: "4px 0 14px" }}>
              Select three different mentors. Once submitted, these preferences are
              locked.
            </p>
            <button
