  ...
  try {
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        totalCapacity: mentorList.reduce((total, mentor) => total + mentor.capacity, 0),
        assigned: allocationList.length,
        unassigned: submittedMentees.filter((mentee) => !allocatedMenteeIds.has(mentee.id)).length,
        availableCapacity: mentorList.reduce((total, mentor) => total + Math.max(mentor.capacity - (allocationsByMentor.get(mentor.id) ?? 0), 0), 0),
      ...
      mentorLoads: mentorList.map((mentor) => ({
